...
def get_pending_command(machine_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT id, machine_id, recipe, execute_allowed, meta, status, created_at FROM iot_commands WHERE machine_id = %s AND status = 'pending' ORDER BY created_at ASC LIMIT 1",
            (machine_id,),
        )
        row = cur.fetchone()
        cur.close()
        ...
        logger.error("Error getting pending command: %s", exc)
        return None
...
def update_command_status(command_id: int, status: str, meta: Optional[Dict] = None) -> bool:
    ...
    try:
        ...
        logger.error("Error updating command status: %s", exc)
        return False
