...
export async function POST(request: Request) {
	try {
		...
		const backendResponse = await fetch(`${API_BASE}/api/admin/mfa/verify`, {
			method: "POST",
			headers: {
		...
		const data = { ...(parsed as VerifyPayload) };
		...
		return NextResponse.json({ error: "Admin MFA proxy failed" }, { status: 500 });
	}
}
