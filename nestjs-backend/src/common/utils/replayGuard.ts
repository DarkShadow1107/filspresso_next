...
export function normalizeOperationId(value: any) {
...
export function extractOperationId(req: any) {
...
export function resolveActorId(req: any) {
  if (req.user?.id) return `user:${req.user.id}`;
  if (req.adminSession?.userId) return `admin:${req.adminSession.userId}`;
  if (req.serviceIdentity?.service_name) return `service:${String(req.serviceIdentity.service_name).slice(0, 64)}`;
  if (req.ip) return `ip:${String(req.ip).slice(0, 64)}`;
  return "anonymous";
}

export async function reserveReplayOperation(pool: any, options: any = {}) {
  ...
  if (!operationId) return { ok: false, reason: "invalid_operation_id" };
  ...
  if (result.rowCount === 0) return { ok: false, reason: "replay_detected", operationId };
