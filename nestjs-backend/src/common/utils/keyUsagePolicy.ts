...
function normalizeRuleSet(value: any) {
  if (value === "*") {
  ...
  return {
    wildcard: false,
    values: new Set(list.map((entry: any) => String(entry || "").trim()).filter(Boolean)),
  };
}
...
function buildPolicies() {
  const overrides: any = parseOverridePolicies();
...
export const KEY_USAGE_POLICIES = buildPolicies();

function matchesRule(ruleSet: any, value: string) {
  if (ruleSet.wildcard) {
    return true;
  }
...
export function assertKeyUsage(options: any = {}) {
