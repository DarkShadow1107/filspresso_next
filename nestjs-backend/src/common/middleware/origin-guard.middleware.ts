...
const csrfRequireOriginForCookie =
	String(process.env.CSRF_REQUIRE_ORIGIN_FOR_COOKIE || "true")
		.trim()
		.toLowerCase() === "true";
const csrfEnforceFetchMetadata =
...
function allowLocalhostOrigin(origin: string): boolean {
...
function hasCookies(req: Request): boolean {
...
export function originGuardMiddleware(req: RequestWithId, res: Response, next: NextFunction): void {
