...
export class DatabaseService {
  ...
  constructor() {
    ...
    if (STRICT_SERVICE_DB_CREDENTIALS) {
      ...
      if (BACKEND_DB_USER === ROOT_DB_USER) throw new Error("STRICT_SERVICE_DB_CREDENTIALS=true requires BACKEND_DB_USER to differ from DB_USER");
      if (!BACKEND_DB_PASSWORD) throw new Error("STRICT_SERVICE_DB_CREDENTIALS=true requires BACKEND_DB_PASSWORD or BACKEND_DB_PASSWORD_FILE");
    }
