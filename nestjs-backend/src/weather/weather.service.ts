...
type WeatherRec = { type: string; drink: string; message: string; icon: string } | null;

function getCoffeeRecommendation(temperature: number | null, weatherCode = 0, hour = new Date().getHours()): WeatherRec {
  ...
  if (isSnowyStormy) {
    if (isMorning) return { type: "hot", drink: "Double Espresso", message: "Brave the snow with a powerful double espresso!", icon: "☕" };
    if (isAfternoon) return { type: "hot", drink: "Irish Coffee", message: "A warm Irish coffee is perfect for a stormy afternoon.", icon: "☕" };
    if (isEvening) return { type: "hot", drink: "Mocha", message: "Nothing beats a rich mocha on a snowy evening.", icon: "☕" };
    return { type: "hot", drink: "Ristretto", message: "A short, intense ristretto to warm you through the night.", icon: "☕" };
  }
  if (isRainy) {
    if (isMorning) return { type: "hot", drink: "Cappuccino", message: "Start your rainy morning with a creamy cappuccino.", icon: "☕" };
    if (isAfternoon) return { type: "hot", drink: "Flat White", message: "A smooth flat white to brighten a grey afternoon.", icon: "☕" };
    if (isEvening) return { type: "hot", drink: "Latte", message: "A warm latte — the perfect companion for a rainy evening.", icon: "🥛" };
    return { type: "hot", drink: "Lungo", message: "A slow lungo to wind down on a rainy night.", icon: "☕" };
  }
  if (isCold) {
    if (isMorning) return { type: "hot", drink: "Double Espresso", message: "Kick off a cold morning with a warming double espresso!", icon: "☕" };
    if (isAfternoon) return { type: "hot", drink: "Cappuccino", message: "Wrap your hands around a hot cappuccino this afternoon.", icon: "☕" };
  ...
  if (isCool) {
    ...
    if (isAfternoon && isSunny) return { type: "warm", drink: "Flat White", message: "A sunny but cool afternoon — perfect for a flat white.", icon: "☀️" };
    if (isAfternoon) return { type: "hot", drink: "Latte", message: "A comforting latte for a cool afternoon.", icon: "🥛" };
    if (isEvening) return { type: "hot", drink: "Cappuccino", message: "End your day with a classic cappuccino.", icon: "☕" };
    return { type: "hot", drink: "Espresso", message: "A quick espresso to energize the night.", icon: "☕" };
  }
  if (isMild) {
    if (isMorning) return { type: "warm", drink: "Latte", message: "A pleasant morning calls for a smooth latte.", icon: "🥛" };
    if (isAfternoon) return { type: "warm", drink: "Cortado", message: "Balance the mild afternoon with a perfectly balanced cortado.", icon: "☕" };
    if (isEvening) return { type: "cold", drink: "Iced Latte", message: "Cool down into the evening with a refreshing iced latte!", icon: "🧊" };
    return { type: "warm", drink: "Cold Brew", message: "A smooth cold brew to sip through the night.", icon: "❄️" };
  }
  if (isWarm) {
    if (isMorning) return { type: "cold", drink: "Iced Americano", message: "Beat the morning heat with a crisp iced americano!", icon: "🧊" };
    if (isAfternoon && isSunny) return { type: "cold", drink: "Cold Brew", message: "Hot sunny afternoon? A cold brew is your best friend.", icon: "❄️" };
    if (isAfternoon) return { type: "cold", drink: "Iced Cappuccino", message: "Stay cool this afternoon with an iced cappuccino.", icon: "🧊" };
    if (isEvening) return { type: "cold", drink: "Nitro Cold Brew", message: "Refresh your evening with a silky nitro cold brew.", icon: "❄️" };
...
export class WeatherService {
  ...
  async getWeather(lat: number, lon: number, clientIp: string): Promise<unknown> {
    ...
    try {
      ...
      if (geoRes.ok) {
        ...
        if (geoData.status === "success") { city = geoData.city || null; country = geoData.country || null; }
      }
    ...
    try {
      ...
      if (cached.rows[0]) {
        ...
        if (ageMinutes < CACHE_TTL_MINUTES) {
          const data = cached.rows[0].data as Record<string, unknown>;
      ...
      if (!response.ok) throw new BadRequestException({ error: "Weather API unavailable" });

      const json = await response.json() as Record<string, unknown>;
      const nowHour = new Date().getHours();
      const current = json.current as Record<string, number> | null;
      const rawOutput = {
      ...
      await client.query(
        `INSERT INTO weather_cache (cache_key, data, timestamp) VALUES ($1, $2, NOW())
  ...
  async getRecommendation(lat: number, lon: number): Promise<unknown> {
    const safeLat = Number.isFinite(lat) ? lat : DEFAULT_LAT;
    const safeLon = Number.isFinite(lon) ? lon : DEFAULT_LON;
    const url = new URL("https://api.open-meteo.com/v1/forecast");
    url.searchParams.set("latitude", safeLat.toString());
