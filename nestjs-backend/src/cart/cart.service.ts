...
const ACTIVE_CART_RESERVATION_MINUTES = Math.max(
  1,
  Number.parseInt(process.env.CART_RESERVATION_MINUTES || "20", 10),
);
const CART_STOCK_BUFFER_UNITS = Math.max(
  0,
  Number.parseInt(process.env.CART_STOCK_BUFFER_UNITS || "0", 10),
);
...
export class CartService {
  ...
  private resolveInventoryTable(productType: string): string | null {
  ...
  private async getStockAvailability(
    ...
    { accountId, productType, productId, lockRow }: {
      accountId: number; productType: string; productId: string; lockRow?: boolean;
    },
  ) {
    ...
    const stockResult = await client.query(
      `SELECT stock FROM ${inventoryTable} WHERE product_id = $1${lockRow ? " FOR UPDATE" : ""}`,
      [productId],
    );
  ...
  private async getUserTier(client: import("pg").PoolClient, accountId: number): Promise<string> {
    try {
      try {
        const r = await client.query("SELECT current_tier FROM member_status WHERE account_id = $1", [accountId]);
        if (r.rows[0]?.current_tier) return String(r.rows[0].current_tier);
      } catch (e: unknown) {
        if ((e as { code?: string }).code !== "42P01") throw e;
      }
      const ar = await client.query("SELECT created_at FROM accounts WHERE id = $1", [accountId]);
      if (!ar.rows[0]) return "None";
      const accountCreated = new Date(ar.rows[0].created_at as string);
      const now = new Date();
      ...
      const cr = await client.query(
        `SELECT COALESCE(SUM(oi.quantity), 0) as total_sleeves
         ...
         WHERE o.account_id = $1 AND o.created_at >= $2 AND oi.product_type = 'capsule'`,
        [accountId, periodStart.toISOString()],
      );
      const total = (Number(cr.rows[0]?.total_sleeves) || 0) * 10;
      if (total >= 7000) return "Ambassador";
      if (total >= 4000) return "Virtuoso";
      if (total >= 2000) return "Master";
      if (total >= 750) return "Expert";
      if (total >= 1) return "Connoisseur";
      return "None";
    } catch {
      return "None";
    }
    ...
    try {
      const result = await client.query(
        `SELECT id, product_type, product_id, product_name, product_image, unit_price, quantity, created_at
         FROM cart_items WHERE account_id = $1 ORDER BY created_at DESC`,
        [accountId],
      );
  ...
  async addItem(accountId: number, body: {
    productType?: string; productId?: string; productName?: string;
    productImage?: string; unitPrice?: number; quantity?: number;
  }) {
    ...
    if (!productType || !productId || !productName || unitPrice === undefined) {
      throw new BadRequestException({ error: "Product type, ID, name and price are required" });
    }
    if (!["capsule", "machine", "accessory"].includes(String(productType))) {
      throw new BadRequestException({ error: "Invalid product type" });
    }
    if (!Number.isInteger(normalizedQty) || normalizedQty < 1) {
      throw new BadRequestException({ error: "Quantity must be an integer of at least 1" });
    }
    const pool = this.db.getPool();
    const client = await pool.connect();
    try {
      ...
      const existing = await client.query(
        "SELECT id, quantity FROM cart_items WHERE account_id = $1 AND product_type = $2 AND product_id = $3",
        [accountId, productType, productId],
      );
      ...
      if (availability.missingProduct) { await client.query("ROLLBACK"); throw new NotFoundException({ error: "Product not found in stock inventory" }); }
      if (availability.tracked && targetQty > availability.availableQuantity) {
        await client.query("ROLLBACK");
        throw new ConflictException({ error: `Only ${availability.availableQuantity} units currently available.`, availableQuantity: availability.availableQuantity, requestedQuantity: targetQty });
      }
      if (existingRow) {
      ...
      await client.query(
        "INSERT INTO cart_items (account_id, product_type, product_id, product_name, product_image, unit_price, quantity) VALUES ($1, $2, $3, $4, $5, $6, $7)",
        [accountId, productType, productId, productName, productImage || null, unitPrice, normalizedQty],
      );
      await client.query("COMMIT");
  ...
  async updateItem(accountId: number, itemId: number, quantity: number) {
    const normalizedQty = Number.parseInt(String(quantity), 10);
    if (!Number.isInteger(normalizedQty) || normalizedQty < 1) {
      throw new BadRequestException({ error: "Quantity must be an integer of at least 1" });
    }
    ...
    try {
      ...
      if (!item) { await client.query("ROLLBACK"); throw new NotFoundException({ error: "Cart item not found" }); }
      ...
      if (availability.tracked && normalizedQty > availability.availableQuantity) {
        await client.query("ROLLBACK");
        throw new ConflictException({ error: `Only ${availability.availableQuantity} units available.`, availableQuantity: availability.availableQuantity });
      }
  ...
  async removeItem(accountId: number, itemId: number) {
    const pool = this.db.getPool();
    const client = await pool.connect();
    try {
      const result = await client.query("SELECT id FROM cart_items WHERE id = $1 AND account_id = $2", [itemId, accountId]);
      if (!result.rows[0]) throw new NotFoundException({ error: "Cart item not found" });
