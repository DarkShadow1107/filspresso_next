import { Injectable, HttpException, BadRequestException, NotFoundException, ForbiddenException, UnauthorizedException } from "@nestjs/common";
...
export class AdminService {
  ...
  private normalizeAdminIdentifier(value: any) { return String(value || "").trim().toLowerCase(); }
  private isRestrictedAdminTable(table: string) { return RESTRICTED_ADMIN_TABLES.has(this.normalizeAdminIdentifier(table)); }
  private isSensitiveAdminColumn(columnName: string) {
  ...
  private getVisibleAdminColumns(columns: any[]) { return columns.filter(c => !this.isSensitiveAdminColumn(c.name)); }
  ...
  private getCookieValue(req: Request, name: string) {
    ...
    for (const part of rawCookie.split(";")) {
      ...
      if (key && key.trim() === name) return decodeURIComponent(rest.join("=").trim() || "");
    }
    return "";
  }

  private getSensitiveViewSecret() { return String(process.env.JWT_SECRET || process.env.ENCRYPTION_KEY || "").trim(); }

  private buildSensitiveViewTokenPayload(session: any) {
  ...
  private signSensitiveViewToken(payload: any) {
  ...
  private verifySensitiveViewToken(token: string) {
    const secret = this.getSensitiveViewSecret();
    ...
    try {
      ...
      if (!payload || payload.scope !== "admin_sensitive_view" || !Number.isFinite(payload.exp) || payload.exp < Date.now()) return null;
      return payload;
    } catch { return null; }
  }

  private setSensitiveViewCookie(res: Response, token: string) {
  ...
  private clearSensitiveViewCookie(res: Response) {
  ...
  public isAllowedImage(file: any) {
    const ext = path.extname(file.originalname || "").toLowerCase().replace(".", "");
    const mime = (file.mimetype || "").toLowerCase();
    if (VALID_IMAGE_EXTENSIONS.has(ext)) return true;
    if (mime.includes("image/avif") || mime.includes("image/webp") || mime.includes("image/png") || mime.includes("jpeg") || mime.includes("image/svg+xml")) return true;
    return false;
  }

  public resolveImageTarget(productType: string, category: string) {
  ...
  private generateAdminToken() { return `admin_${crypto.randomBytes(32).toString("hex")}`; }
  private normalizeAdminLoginKey(value: any) { return String(value || "").trim().toLowerCase().slice(0, 254); }
  ...
  private async getAdminLockState(client: any, loginKey: string) {
    if (RELAX_ADMIN_LIMITS_IN_DEV) return { isLocked: false, retryAfterSeconds: 0, failedAttempts: 0 };
    ...
    if (!lockUntil || Number.isNaN(lockUntil.getTime()) || lockUntil <= new Date()) return { isLocked: false, retryAfterSeconds: 0, failedAttempts: Number(row.failed_attempts) || 0 };
  ...
  private async recordAdminFailedAttempt(client: any, loginKey: string) {
    ...
    if (lockSeconds > 0) await client.query("UPDATE auth_login_attempts SET lock_until = NOW() + (($1)::text || ' seconds')::interval WHERE login_key = $2", [lockSeconds, loginKey]);
    return { failedAttempts, lockSeconds };
  }

  private async clearAdminFailedAttempts(client: any, loginKey: string) { await client.query("DELETE FROM auth_login_attempts WHERE login_key = $1", [loginKey]); }

  private async logAdminSecurityEvent(client: any, eventType: string, payload: any = {}) {
  ...
  private hashChallengeToken(token: string) { return crypto.createHash("sha256").update(String(token || "")).digest("hex"); }
  ...
  private decodeBase32(base32: string) {
  ...
  private generateTotp(secret: string, timestampMs = Date.now()) {
  ...
  private buildOtpAuthUrl(secret: string, username: string) {
  ...
  private async generateAdminQrDataUrl(otpauthUrl: string) {
    try { return await QRCode.toDataURL(otpauthUrl, { errorCorrectionLevel: "M", margin: 1, width: 220 }); } catch { return null; }
  }

  private async createAdminMfaChallenge(client: any, opts: any) {
  ...
  private async getActiveMfaChallenge(client: any, token: string) {
    const tokenHash = this.hashChallengeToken(token);
  ...
  private async invalidateExpiredChallenges(client: any) { await client.query("DELETE FROM admin_mfa_challenges WHERE consumed_at IS NOT NULL OR expires_at < NOW()"); }

  private async createAdminSession(client: any, user: any, req: Request) {
  ...
  private isSafeReadOnlyAdminQuery(sql: string) {
    const normalized = String(sql || "").trim();
    if (!normalized) return false;
    if (normalized.length > ADMIN_QUERY_MAX_LENGTH) return false;
    if (normalized.includes(";") || /--|\/\*/.test(normalized)) return false;
    const upper = normalized.toUpperCase();
    if (!(upper.startsWith("SELECT") || upper.startsWith("WITH") || upper.startsWith("SHOW") || upper.startsWith("DESCRIBE"))) return false;
    if (/\b(INSERT|UPDATE|DELETE|DROP|ALTER|TRUNCATE|CREATE|GRANT|REVOKE|MERGE|CALL|COPY|DO|EXECUTE)\b/.test(upper)) return false;
    return true;
  }

  private isSafeSqlIdentifier(value: string) { return ADMIN_IDENTIFIER_PATTERN.test(String(value || "")); }

  async authenticateAdmin(req: any, res: Response) {
  ...
  async login(req: Request, res: Response) {
    const ip = this.getRequestIp(req);
    if (!this.isIpAllowed(ip)) { res.status(403).json({ error: "Admin access is restricted from this network" }); return; }
    ...
    try {
      ...
      if (!username || !password || password.length > 128) throw new BadRequestException({ error: "Username and password are required" });
      ...
      if (lockState.isLocked) {
        ...
        throw new HttpException({ error: "Too many failed admin login attempts. Please try again later.", retryAfterSeconds: lockState.retryAfterSeconds }, 429);
      }
      ...
      const totp: any = { secret: enrollmentSecret, issuer: ADMIN_MFA_ISSUER, accountName: user.username, otpauthUrl };
      if (includeQrCode) totp.qrDataUrl = await this.generateAdminQrDataUrl(otpauthUrl);
  ...
  async mfaVerify(req: Request, res: Response) {
    ...
    try {
      ...
      if (!challengeToken || !code) throw new BadRequestException({ error: "challengeToken and code are required" });

      await this.invalidateExpiredChallenges(client);
      const challenge = await this.getActiveMfaChallenge(client, challengeToken);
      if (!challenge || challenge.role !== "admin") throw new UnauthorizedException({ error: "Invalid or expired MFA challenge" });

      const expiresAt = new Date(challenge.expires_at);
      if (!expiresAt || Number.isNaN(expiresAt.getTime()) || expiresAt <= new Date()) throw new UnauthorizedException({ error: "Invalid or expired MFA challenge" });
      if (Number(challenge.attempts) >= Number(challenge.max_attempts)) throw new HttpException({ error: "Too many MFA attempts. Start login again." }, 429);
      ...
      if (!secret) throw new UnauthorizedException({ error: "MFA challenge is no longer valid" });
      ...
      if (!validCode) {
        ...
        throw new UnauthorizedException({ error: "Invalid MFA code" });
      }
  ...
  async logout(req: any) {
    ...
    const pool = this.db.getPool();
    const client = await pool.connect();
    try { await client.query("DELETE FROM user_sessions WHERE session_token = $1", [token]); } finally { client.release(); }
  ...
  async sensitiveUnlock(req: any, res: Response) {
    ...
    try {
      ...
      if (!admin || !(await passwords.verifyPassword(password, admin.password_hash))) throw new UnauthorizedException({ error: "Invalid password" });
      const token = this.signSensitiveViewToken(this.buildSensitiveViewTokenPayload(req.adminSession));
      if (!token) throw new HttpException({ error: "Unable to enable sensitive mode" }, 500);
  ...
  async sensitiveLock(req: any, res: Response) {
    this.clearSensitiveViewCookie(res);
    return { status: "success", sensitiveViewEnabled: false };
  }

  private async assertPublicTable(client: any, table: string, includeSensitive = false) {
    if (!includeSensitive && this.isRestrictedAdminTable(table)) return false;
  ...
  private async getTableColumns(client: any, table: string) {
    const result = await client.query(
    ...
    return result.rows.map((column: any) => ({ name: column.name, type: column.type, nullable: column.nullable === "YES", isPrimary: column.key_type === "PRI", isAutoIncrement: column.is_auto_increment, defaultValue: column.default_value, maxLength: column.max_length ? Number(column.max_length) : null }));
  }

  private async getPrimaryKey(client: any, table: string) {
  ...
  private sanitizeRowData(data: any, columns: any[], includeSensitive = false) {
    ...
    for (const [key, value] of Object.entries(data || {})) {
      if (editableColumns.has(key) && (includeSensitive || !this.isSensitiveAdminColumn(key))) sanitized[key] = value;
    }
  ...
  async getTables(req: any) {
    ...
    try {
      ...
      if (metaResult.rows.length === 0) return { status: "success", tables: [] };
      const visibleTables = includeSensitive ? metaResult.rows : metaResult.rows.filter((t: any) => !this.isRestrictedAdminTable(t.name));
      if (visibleTables.length === 0) return { status: "success", tables: [] };
      const countQuery = visibleTables.map((t: any) => `SELECT '${t.name}' as name, COUNT(*)::bigint as row_count FROM "${t.name}"`).join(" UNION ALL ");
      const countResult = await client.query(countQuery);
      const countMap: any = {};
      for (const row of countResult.rows) countMap[row.name] = Number(row.row_count);
      return { status: "success", tables: visibleTables.map((t: any) => ({ name: t.name, rowCount: countMap[t.name] ?? 0, comment: t.comment || "" })) };
    } finally { client.release(); }
  }

  async getTableInfo(req: any) {
    const { table } = req.params;
    if (!this.isSafeSqlIdentifier(table)) throw new BadRequestException({ error: "Invalid table name" });
    const includeSensitive = Boolean(req.adminSensitiveView);
    const pool = this.db.getPool();
    const client = await pool.connect();
    try {
      const tableExists = await this.assertPublicTable(client, table, includeSensitive);
      if (!tableExists) throw new NotFoundException({ error: "Table not found" });
      ...
      const visibleColumns = includeSensitive ? columnsResult.rows : columnsResult.rows.filter((column: any) => !this.isSensitiveAdminColumn(column.name));
      return { status: "success", table, primaryKey, columns: visibleColumns.map((c: any) => ({ name: c.name, type: c.type, nullable: c.nullable === "YES", isPrimary: c.key_type === "PRI", isAutoIncrement: c.is_auto_increment, defaultValue: c.default_value, maxLength: c.max_length ? Number(c.max_length) : null, comment: c.comment || "" })) };
    } finally { client.release(); }
  }

  async getTableData(req: any) {
    const { table } = req.params;
    if (!this.isSafeSqlIdentifier(table)) throw new BadRequestException({ error: "Invalid table name" });
    const includeSensitive = Boolean(req.adminSensitiveView);
    ...
    if (!this.isSafeSqlIdentifier(requestedSortBy)) throw new BadRequestException({ error: "Invalid sort column" });
    ...
    try {
      ...
      if (visibleColumns.length === 0) throw new NotFoundException({ error: "Table not found" });

      const columnNames = new Set(visibleColumns.map((column: any) => column.name));
      const sortBy = columnNames.has(requestedSortBy) ? requestedSortBy : await this.getPrimaryKey(client, table);

      let countQuery = `SELECT COUNT(*) as total FROM "${table}"`;
      let dataQuery = `SELECT ${visibleColumns.map((c: any) => `"${c.name}"`).join(", ")} FROM "${table}"`;
      const params: any[] = [];
      let paramIdx = 1;
      ...
      if (search) {
        ...
        if (columnsResult.rows.length > 0) {
          const searchableColumns = includeSensitive ? columnsResult.rows : columnsResult.rows.filter((c: any) => columnNames.has(c.column_name));
          const searchConditions = searchableColumns.map((c: any) => `"${c.column_name}"::text ILIKE $${paramIdx++}`).join(" OR ");
          if (searchConditions) {
  ...
  async insertRow(req: any) {
    ...
    try {
      ...
      if (!sanitized || Object.keys(sanitized).length === 0) throw new BadRequestException({ error: "No editable data provided" });

      if (table === "accounts" && sanitized.password_hash) sanitized.password_hash = await passwords.hashPassword(sanitized.password_hash);
  ...
  async updateRow(req: any) {
    ...
    try {
      ...
      if (table === "accounts" && sanitized.password_hash) {
        ...
        if (currentUser && currentUser.password_hash === sanitized.password_hash) delete sanitized.password_hash;
        else sanitized.password_hash = await passwords.hashPassword(sanitized.password_hash);
      }
      ...
      if (result.rowCount === 0) throw new NotFoundException({ error: "Row not found" });
      return { status: "success", message: "Row updated successfully", affectedRows: Number(result.rowCount) };
    } finally { client.release(); }
  }

  async deleteRow(req: any) {
    const { table, id } = req.params;
    if (!this.isSafeSqlIdentifier(table)) throw new BadRequestException({ error: "Invalid table name" });
    const includeSensitive = Boolean(req.adminSensitiveView);
    const pool = this.db.getPool();
  ...
  async executeQuery(req: any) {
    const { sql } = req.body;
    if (!sql) throw new BadRequestException({ error: "SQL query is required" });
    if (!this.isSafeReadOnlyAdminQuery(sql)) throw new ForbiddenException({ error: "Only single-statement read-only queries are allowed (SELECT/WITH/SHOW/DESCRIBE)." });
  ...
  async getSecurityEvents(req: any) {
  ...
  async clearLockout(req: any) {
    const loginKey = String(req.params.loginKey || "").trim().toLowerCase().slice(0, 254);
    if (!loginKey) throw new BadRequestException({ error: "loginKey is required" });
    const pool = this.db.getPool();
    const client = await pool.connect();
    try {
