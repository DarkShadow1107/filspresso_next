...
const secretsUtils = __importStar(require("../common/utils/secrets"));
...
let CryptoService = CryptoService_1 = class CryptoService {
    ...
    createRustCryptoServiceAssertion(scope, operationId = "") {
        if (!this.serviceAssertionPrivateKey)
            return "";
        try {
    ...
    buildRustCryptoUpstreamHeaders(scope, operationId = "") {
    ...
    async callRustCrypto(pathname, payload, options = {}) {
    ...
    async appendMpcTranscript(client, options) {
        ...
        await client.query(`INSERT INTO mpc_quorum_transcripts (session_id, participant_account_id, event_type, event_payload, signature)
       VALUES ($1, $2, $3, $4::jsonb, $5)`, [options.sessionId, options.participantAccountId || null, String(options.eventType || "mpc.event").slice(0, 48), JSON.stringify(payload), signature || null]);
    }
    async enforceMpcTranscriptRateLimit(client, accountId) {
        ...
        if (count >= MPC_TRANSCRIPT_RATE_LIMIT_PER_MINUTE)
            throw new common_1.HttpException("MPC transcript rate limit exceeded", 429);
    }
    async getHealth() {
        try {
            const upstream = await this.callRustCrypto("/health");
            if (!upstream.ok)
                throw new common_1.ServiceUnavailableException({ error: "Rust crypto service unavailable", upstream });
            return { upstream: upstream.data || {} };
        }
        catch (error) {
            throw new common_1.ServiceUnavailableException({ error: "Failed to reach rust crypto service" });
        }
    }
    async createCommitment(operationId, domain, payload, payloadJson) {
        if (!domain)
            throw new common_1.BadRequestException({ error: "domain is required" });
        const upstreamHeaders = this.buildRustCryptoUpstreamHeaders(RUST_CRYPTO_SERVICE_ASSERTION_SCOPE_COMMITMENT, operationId);
        if (!upstreamHeaders)
            throw new common_1.ServiceUnavailableException({ error: "Rust crypto service assertion is required but could not be issued" });
        ...
        if (!upstream.ok)
            throw new common_1.ServiceUnavailableException({ error: "Rust crypto commitment failed", upstream: upstream.data });
    ...
    async verifyCommitment(operationId, domain, commitment, payload, payloadJson) {
        if (!domain || !commitment)
            throw new common_1.BadRequestException({ error: "domain and commitment_sha3_256 are required" });
        ...
        if (!upstream.ok)
            throw new common_1.ServiceUnavailableException({ error: "Rust crypto verification failed", upstream: upstream.data });
    ...
    async createThresholdOperation(userId, operationId, body) {
        const operationType = String(body?.operation_type || body?.operationType || "").trim();
        if (!operationType)
            throw new common_1.BadRequestException({ error: "operation_type is required" });
        ...
        try {
            ...
            const inserted = await client.query(`INSERT INTO threshold_operations (operation_id, operation_type, payload, required_approvals, created_by, expires_at)
         VALUES ($1, $2, $3::jsonb, $4, $5, NOW() + (($6)::text || ' seconds')::interval) RETURNING *`, [operationId, operationType.slice(0, 64), JSON.stringify(payload), requiredApprovals, userId, expiresInSeconds]);
    ...
    async approveThresholdOperation(userId, actionOperationId, thresholdOperationId) {
        if (!Number.isInteger(thresholdOperationId) || thresholdOperationId <= 0)
            throw new common_1.BadRequestException({ error: "Invalid threshold operation id" });
        const pool = this.db.getPool();
        const client = await pool.connect();
        try {
            await client.query("BEGIN");
            ...
            if (!operationRow) {
                await client.query("ROLLBACK");
                throw new common_1.NotFoundException({ error: "Threshold operation not found" });
            }
            if (["cancelled", "rejected", "executed"].includes(String(operationRow.status))) {
                await client.query("ROLLBACK");
                throw new common_1.ConflictException({ error: `Operation is already ${operationRow.status}` });
            }
            if (operationRow.expires_at && new Date(operationRow.expires_at).getTime() <= Date.now()) {
                ...
                await client.query("COMMIT");
                throw new common_1.ConflictException({ error: "Operation is expired" });
            }
            const approvalInsert = await client.query(`INSERT INTO threshold_operation_approvals (threshold_operation_id, account_id, operation_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING RETURNING id`, [thresholdOperationId, userId, actionOperationId]);
    ...
    async executeThresholdOperation(userId, thresholdOperationId) {
        if (!Number.isInteger(thresholdOperationId) || thresholdOperationId <= 0)
            throw new common_1.BadRequestException({ error: "Invalid threshold operation id" });
        const pool = this.db.getPool();
        const client = await pool.connect();
        try {
            ...
            if (String(operationRow.status) !== "approved") {
                await client.query("ROLLBACK");
                throw new common_1.ConflictException({ error: "Operation is not approved" });
            }
            const executed = await client.query(`UPDATE threshold_operations SET status = 'executed', executed_at = NOW(), executed_by = $2, updated_at = NOW() WHERE id = $1 RETURNING *`, [thresholdOperationId, userId]);
            await client.query("COMMIT");
    ...
    async getThresholdOperation(thresholdOperationId) {
        if (!Number.isInteger(thresholdOperationId) || thresholdOperationId <= 0)
            throw new common_1.BadRequestException({ error: "Invalid threshold operation id" });
        const pool = this.db.getPool();
        ...
        if (!operation)
            throw new common_1.NotFoundException({ error: "Threshold operation not found" });
        const approvals = await pool.query(`SELECT a.account_id, acc.username, a.created_at FROM threshold_operation_approvals a JOIN accounts acc ON acc.id = a.account_id WHERE a.threshold_operation_id = $1 ORDER BY a.created_at ASC`, [thresholdOperationId]);
        return { status: "success", operation: toThresholdOperationResponse(operation), approvals: approvals.rows };
    }
    // ZK CIRCUITS
    async registerZkCircuit(userId, body) {
        ...
        if (!circuitName || !circuitVersion || !verificationKey)
            throw new common_1.BadRequestException({ error: "circuitName, circuitVersion, and verificationKey are required" });
        if (!["proposed", "active", "deprecated", "revoked"].includes(governanceStatus))
            throw new common_1.BadRequestException({ error: "governanceStatus must be proposed|active|deprecated|revoked" });
        const pool = this.db.getPool();
        const verificationKeyHash = sha3Hex(verificationKey);
        const result = await pool.query(`INSERT INTO zk_circuit_registry (circuit_name, circuit_version, governance_status, verification_key, verification_key_hash, created_by)
       VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (circuit_name, circuit_version) DO UPDATE SET governance_status = EXCLUDED.governance_status, verification_key = EXCLUDED.verification_key, verification_key_hash = EXCLUDED.verification_key_hash, updated_at = NOW() RETURNING *`, [circuitName, circuitVersion, governanceStatus, verificationKey, verificationKeyHash, userId]);
        return { status: "success", circuit: result.rows[0] };
    }
    async updateZkCircuitStatus(id, nextStatus) {
        if (!Number.isInteger(id) || id <= 0)
            throw new common_1.BadRequestException({ error: "Invalid circuit id" });
        if (!["proposed", "active", "deprecated", "revoked"].includes(nextStatus))
            throw new common_1.BadRequestException({ error: "status must be proposed|active|deprecated|revoked" });
        const transitions = { proposed: ["active", "revoked"], active: ["deprecated", "revoked"], deprecated: ["revoked"], revoked: [] };
        const pool = this.db.getPool();
        const current = await pool.query("SELECT * FROM zk_circuit_registry WHERE id = $1", [id]);
        const row = current.rows[0];
        if (!row)
            throw new common_1.NotFoundException({ error: "Circuit not found" });
        ...
        if (currentStatus !== nextStatus && !(transitions[currentStatus] || []).includes(nextStatus))
            throw new common_1.ConflictException({ error: `Invalid governance transition from ${currentStatus} to ${nextStatus}` });
    ...
    // ZK PROOFS
    async generateZkProof(userId, operationId, body) {
        const circuitName = String(body?.circuitName || "").trim().slice(0, 96);
        const circuitVersion = String(body?.circuitVersion || "").trim().slice(0, 32);
        if (!circuitName || !circuitVersion)
            throw new common_1.BadRequestException({ error: "circuitName and circuitVersion are required" });
        ...
        if (Buffer.byteLength(witnessCanonical, "utf8") > ZK_MAX_WITNESS_BYTES)
            throw new common_1.HttpException("Witness payload is too large", 413);
        const pool = this.db.getPool();
        ...
        if (!circuitRow)
            throw new common_1.NotFoundException({ error: "Active circuit version not found" });
        ...
        const created = await pool.query(`INSERT INTO zk_proof_sessions (operation_id, account_id, circuit_name, circuit_version, public_inputs, witness_hash, proof_hash, typed_event)
       VALUES ($1, $2, $3, $4, $5::jsonb, $6, $7, $8::jsonb) RETURNING *`, [operationId, userId, circuitName, circuitVersion, JSON.stringify(publicInputs), witnessHash, proofHash, JSON.stringify(typedEvent)]);
        return { status: "success", proof: created.rows[0] };
    }
    async verifyZkProof(userId, operationId, proofId) {
        if (!Number.isInteger(proofId) || proofId <= 0)
            throw new common_1.BadRequestException({ error: "Invalid proof id" });
        const pool = this.db.getPool();
        const client = await pool.connect();
        try {
            await client.query("BEGIN");
            ...
            if (!proofRow) {
                await client.query("ROLLBACK");
                throw new common_1.NotFoundException({ error: "Proof session not found" });
            }
            ...
            if (!circuitRow) {
                await client.query("ROLLBACK");
                throw new common_1.NotFoundException({ error: "Circuit for proof session not found" });
            }
    ...
    // MPC SESSIONS
    async createMpcSession(userId, operationId, body) {
        const operationType = String(body?.operationType || body?.operation_type || "").trim().slice(0, 64);
        if (!operationType)
            throw new common_1.BadRequestException({ error: "operationType is required" });
        const payload = body?.payload && typeof body.payload === "object" ? body.payload : {};
        const participantsInput = Array.isArray(body?.participants) ? body.participants : [userId];
        const participants = Array.from(new Set(participantsInput.map((v) => Number.parseInt(v, 10)).filter((v) => Number.isInteger(v) && v > 0)));
        if (participants.length === 0)
            throw new common_1.BadRequestException({ error: "participants must contain at least one account id" });
    ...
    async signMpcSession(userId, operationId, sessionId, partialSignatureInput) {
        if (!Number.isInteger(sessionId) || sessionId <= 0)
            throw new common_1.BadRequestException({ error: "Invalid session id" });
        const pool = this.db.getPool();
        const client = await pool.connect();
        try {
            await client.query("BEGIN");
            ...
            if (!session) {
                await client.query("ROLLBACK");
                throw new common_1.NotFoundException({ error: "MPC session not found" });
            }
            if (["finalized", "cancelled", "expired"].includes(String(session.status))) {
                await client.query("ROLLBACK");
                throw new common_1.ConflictException({ error: `MPC session is ${session.status}` });
            }
            ...
            if (!participant) {
                await client.query("ROLLBACK");
                throw new common_1.ForbiddenException({ error: "Account is not a participant for this session" });
            }
            if (participant.signed_at) {
                await client.query("ROLLBACK");
                throw new common_1.ConflictException({ error: "Participant already signed" });
            }
    ...
    async finalizeMpcSession(userId, operationId, sessionId) {
        if (!Number.isInteger(sessionId) || sessionId <= 0)
            throw new common_1.BadRequestException({ error: "Invalid session id" });
        const pool = this.db.getPool();
        const client = await pool.connect();
        try {
            ...
            if (String(session.status) === "finalized") {
                await client.query("ROLLBACK");
                throw new common_1.ConflictException({ error: "MPC session already finalized" });
            }
            ...
            if (signatures.length < Number(session.quorum_required || 2)) {
                await client.query("ROLLBACK");
                throw new common_1.ConflictException({ error: "Session quorum has not been reached" });
            }
