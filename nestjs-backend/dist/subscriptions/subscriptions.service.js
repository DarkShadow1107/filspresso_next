...
function normalizeSubscriptionTier(raw) {
    ...
    if (!normalized || normalized === "none")
        return "free";
    if (Object.prototype.hasOwnProperty.call(TIER_LEVELS, normalized))
        return normalized;
    return "free";
}
function calculateRenewalDate(billingCycle, from = new Date()) {
...
let SubscriptionsService = class SubscriptionsService {
    ...
    async getSubscription(accountId) {
        ...
        try {
            const currentResult = await client.query(`SELECT us.id, us.subscription_tier, us.billing_cycle, us.price_ron,
         ...
         ORDER BY us.is_active DESC, us.created_at DESC LIMIT 1`, [accountId]);
            const currentSub = currentResult.rows[0];
            const scheduledResult = await client.query(`SELECT us.id, us.subscription_tier, us.billing_cycle, us.price_ron,
                us.start_date, us.renewal_date, us.is_active, us.status,
                us.card_id, uc.card_last_four, uc.card_type
         FROM user_subscriptions us
         LEFT JOIN user_cards uc ON us.card_id = uc.id
         WHERE us.account_id = $1 AND us.status = 'scheduled'
         ORDER BY us.start_date ASC LIMIT 1`, [accountId]);
            ...
            const cardOf = (sub) => sub.card_id ? { id: sub.card_id, last_four: sub.card_last_four, type: sub.card_type } : null;
            return {
                subscription: {
                    id: currentSub.id,
                    ...
                    card: cardOf(currentSub),
                },
                scheduled: scheduledSub ? { id: scheduledSub.id, tier: scheduledSub.subscription_tier, billing_cycle: scheduledSub.billing_cycle, price_ron: Number(scheduledSub.price_ron), start_date: scheduledSub.start_date, renewal_date: scheduledSub.renewal_date, status: scheduledSub.status, card: cardOf(scheduledSub) } : null,
            };
        }
        finally {
            client.release();
    ...
    async createSubscription(accountId, body) {
        const { tier, billingCycle, cardId } = body;
        if (!tier || !["free", "basic", "plus", "pro", "max", "ultimate"].includes(String(tier))) {
            throw new common_1.BadRequestException({ error: "Invalid subscription tier" });
        }
        if (tier !== "free" && !["monthly", "annual"].includes(String(billingCycle || ""))) {
            throw new common_1.BadRequestException({ error: "Invalid billing cycle" });
        }
        const price = tier === "free" ? 0 : (SUBSCRIPTION_PRICES[tier]?.[billingCycle] ?? 0);
        ...
        try {
            ...
            const result = await client.query(`INSERT INTO user_subscriptions (account_id, subscription_tier, billing_cycle, price_ron, start_date, renewal_date, is_active, auto_renew, status, card_id) VALUES ($1,$2,$3,$4,$5,$6,TRUE,$7,'active',$8) RETURNING id`, [accountId, tier, billingCycle || "monthly", price, startDate, renewalDate, tier !== "free", cardId || null]);
    ...
    async changeSubscription(accountId, body) {
        const { tier, billingCycle, cardId } = body;
        if (!tier || !["basic", "plus", "pro", "max", "ultimate"].includes(String(tier)))
            throw new common_1.BadRequestException({ error: "Invalid subscription tier" });
        ...
        try {
            ...
            if (!currentSub) {
                ...
                throw new common_1.BadRequestException({ error: "No active subscription to change from" });
            }
            await client.query(`DELETE FROM user_subscriptions WHERE account_id = $1 AND status = 'scheduled'`, [accountId]);
            const price = SUBSCRIPTION_PRICES[tier]?.[billingCycle] ?? 0;
            ...
            const renewalDate = calculateRenewalDate(billingCycle, new Date(endDate));
    ...
    async cancelSubscription(accountId) {
        ...
        try {
            ...
            if (!result.rows[0])
                throw new common_1.NotFoundException({ error: "No active subscription found" });
    ...
    async updateCard(accountId, cardId) {
        if (!cardId)
            throw new common_1.BadRequestException({ error: "Card ID is required" });
        const pool = this.db.getPool();
        const client = await pool.connect();
        try {
    ...
    async toggleAutoRenew(accountId) {
        const pool = this.db.getPool();
        const client = await pool.connect();
        try {
