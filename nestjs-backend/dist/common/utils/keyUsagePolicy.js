...
exports.KEY_USAGE_POLICIES = void 0;
exports.assertKeyUsage = assertKeyUsage;
const SERVICE_ASSERTION_ALLOWED_SCOPES = String(process.env.SERVICE_ASSERTION_ALLOWED_SCOPES ||
...
exports.KEY_USAGE_POLICIES = buildPolicies();
...
function assertKeyUsage(options = {}) {
    ...
    const policy = exports.KEY_USAGE_POLICIES[keyPurpose];
    if (!policy) {
        throw new Error(`No key usage policy registered for ${keyPurpose}`);
    }
    if (!matchesRule(policy.operations, operationType)) {
        throw new Error(`Key usage denied for purpose=${keyPurpose}, operation=${operationType}, keyId=${keyId}: operation not allowed`);
    }
