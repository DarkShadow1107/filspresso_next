...
exports.verifyServiceAssertion = verifyServiceAssertion;
exports.issueServiceAssertion = issueServiceAssertion;
...
const SERVICE_ASSERTION_ISSUER_ALLOWLIST = String(process.env.SERVICE_ASSERTION_ISSUER_ALLOWLIST || "")
    ...
    .filter(Boolean);
const SERVICE_ASSERTION_PUBLIC_KEY = (0, secrets_1.getEnvOrFile)("SERVICE_ASSERTION_PUBLIC_KEY", {
    required: false,
    defaultValue: "",
});
...
function verifyAudience(aud, expectedAudience = SERVICE_ASSERTION_AUDIENCE) {
    if (!expectedAudience)
        return true;
...
function verifyScope(scope, expectedScope) {
...
function verifyIssuerWithAllowlist(iss, allowlist) {
    if (!Array.isArray(allowlist) || allowlist.length === 0)
        return true;
    return allowlist.includes(String(iss || ""));
}
function verifyServiceAssertion(token, options = {}) {
    ...
    const expectedIssuers = Array.isArray(options.expectedIssuers)
        ? options.expectedIssuers.map((entry) => String(entry || "").trim()).filter(Boolean)
...
function issueServiceAssertion(options = {}) {
